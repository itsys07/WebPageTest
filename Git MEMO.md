...
### 正規表現メモ# WEB学習用
## 文字列頻度検出テスト
>>>>>>> 0f8aacb1001b1760d7691351ed4c23e8a6528a17
...
#### CSVダウンロード
- BOM（Byte Oder Mark）：0xEF,0xBB,0xBFをファイルの先頭に付けると文字定義がUTF-8であるとExcelが認識できる。文字出力の互換定義。
- Blob（Binary Large Object）：データ（画像、動画、文字など）をメモリ上のバイナリデータ、メモリ上の一時データを直接扱える。
大きなデータだと出力が膨大になるのでメモリ注意。
ユーザー入力をそのまま受け取りで使うとセキュリティリスク（悪意ある、もしくは予期しないデータ列になる恐れ）があるのでサニタイズが必要。
CSP（Content Security Policy）を設定し外部スクリプトの実行やインラインスクリプトの制限を行うことでXSS攻撃のリスクを軽減する。
- 先頭が = + - @ の場合、' を付与してExcelで数式として解釈されないようにする。
- URL.createObjectURL()
...
- .type.indexOf()
指定した文字列が含まれているかを調べ、見つかった場合は位置（0以上）を返し、見つからない場合は -1 を返す。
- .endsWith()
