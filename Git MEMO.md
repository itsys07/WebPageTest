...
fragmentをつかってcounts要素の値を降順に20要素までをキー、値を1セットにliにtextとして追加していく、appendChildで描画、画面表示、countsが空なら"単語が見つかりませんでした"表示、CSVダウンロードボタン無効のままreturn
...
- 入力
HTMLでselectで選択
JSで正規表現をpresets定義、Elementでmode定義、presets＞mode定義をregexで定義。
textの値からregexに当たる値を空白に置き換えたcleanedを定義。
- 関数addExclude引数word
ElementでexcludeInput定義（'exclude'）から生成
excludeInputの値から（,）切り分け、前後空白取り除いて、空白で切り分けcurrent生成。
currentに含まれていないword（countsのプロパティ）をexcludeInputの末尾に追加。
...
- reduce():Arrayオブジェクトメソッド、アロー関数のプロパティに第一引数が累積値第二引数が現在値で要素ごとの定義された関数が実行される。
